"""
tests/test_search.py — Unit test untuk search & index Let-X
"""
...
def fake_cache(tmp_path, monkeypatch):
    ...
    import letx.config as cfg
    monkeypatch.setattr(cfg, "PACKAGES_CACHE", cache_file)
    monkeypatch.setattr(cfg, "CACHE_TTL", 9999)

    import letx.repo.index as idx
...
def test_fetch_index_from_cache(fake_cache):
    ...
    assert len(fetch_index()) == len(MOCK_PACKAGES)
...
def test_search_by_name(fake_cache):
    from letx.ops.search import search_packages
...
def test_search_partial(fake_cache):
    from letx.ops.search import search_packages
    names = [r["name"] for r in search_packages("zen")]
...
def test_list_all(fake_cache):
    from letx.ops.search import list_packages
...
def test_available_categories(fake_cache):
    from letx.ops.search import available_categories
    assert set(available_categories()) == {"core", "extra", "multilib"}
